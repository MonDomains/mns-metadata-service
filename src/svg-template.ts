...
const fontRobotoBold = importFont(CARD_CANVAS_FONT_PATH, 'font/truetype'); 
const fontUbuntuBold = importFont(CARD_CANVAS_FONT_PATH, 'font/truetype'); 
...
interface CardSVGTemplateFields {
  domain: string;
  tokenId: string,
  registered: Date,
  expired: Date
...
export function createCardSVGfromTemplate({
  domain,
  tokenId,
  registered,
  expired
}: CardSVGTemplateFields) {
  return `
...
<text
  ...
  <text
      ...
      font-size="1.5em"
      fill="white"
      filter="url(#dropShadow)">Expiry: ${new Date(Number(expired) * 1000).toLocaleString("en-US", { month: 'long', day: 'numeric', year: 'numeric' } )} </text>
  <text
      x="24" 
      y="600" 
      font-size="1em"
      fill="white"
      filter="url(#dropShadow)">#${tokenId} </text>
    <defs>
      <style type="text/css">
        @font-face { 
          font-family: "Roboto";
